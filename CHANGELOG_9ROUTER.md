...
## Features
- **Antigravity**: native image generation support (image models tagged kind:image)
- **CodeBuddy CN**: API key auth + credit quota tracker + short model prefix alias "cbcn"
...
## Features
- **Combo Fusion strategy** ΓÇö fans the prompt out to all member models in parallel, then a configurable judge model synthesizes one final answer (quorum-grace, anonymized sources, graceful degradation)
- **Per-combo strategy selector** ΓÇö pick `fallback` / `round-robin` / `fusion` / `capacity` per combo (replaces the old round-robin toggle), with a judge picker for fusion
- **Capacity auto-switch** ΓÇö reorders models per request so images/PDFs route to capable models first
- **Kiro headless API-key auth** (`ksk_`) + direct `claudeΓåökiro` route that avoids the lossy OpenAI two-hop pivot
- **Claude auto-ping** ΓÇö warms the 5h quota window right after reset so a fresh window starts immediately (per-connection toggle)

## Fixes
- **Claude 429**: stop hammering the OAuth usage endpoint ΓÇö cache resetAt, throttle quota refresh to 3 min, cool down after a 429 (chat unaffected)
...
- **Executors**: strip params unsupported by the provider/model (drops deprecated `temperature` for claude-opus-4 ΓåÆ Anthropic 400)
- **Translator**: derive deterministic tool_call ids for gemini/antigravity ΓåÆ OpenAI so function call/response pair correctly (fixes tool-pairing 400s)
...
## Internal
- Large **open-sse / translator refactor** (~40 commits): unified provider/model registry (LiteLLM-style `models[]` + `kind` field, 100 co-located registry files), single-sourced media/OAuth/refresh/token URLs, registry-based dispatch for usage & token-refresh, DRY translator concerns (buildUsage, encodeDataUri, finishReasonMap, chunkBuilder, reasoningDeltaΓÇª), ESM-safe registry init, large-file splits, dead-code removal, and golden/no-regression test gates
...
## Fixes
- Codex: harden streaming timeouts (stall/connect raised to 60s, configurable per-provider), accept `response.done` event, and always emit a terminal `response.failed` + `[DONE]` for Responses passthrough when a stream closes, stalls, or aborts before a terminal event ΓÇö prevents codex clients from hanging (#1648, #1680, #1688, #1618)
...
- Lower stream stall timeout 35s ΓåÆ 30s for faster hang detection
...
## Improvements
- Lower stream stall timeout from 60s ΓåÆ 35s for faster hang detection
...
## Features
- Xiaomi MiMo Token Plan: region selector (Singapore / China / Europe) ΓÇö keys are cluster-specific
...
- rtk: Kiro format tool result compression ΓÇö handle conversationState.history & currentMessage, preserve error results, ~13.6% savings (#1194)

## Fixes
- openclaw: normalize agent.model object form `{primary, fallbacks}` before .startsWith ΓåÆ fix TypeError & 'not configured' status (#1216)
...
## Breaking Changes
- Tunnel public URL changed ΓÇö old tunnel links no longer work, please reconnect to get the new URL
...
- Resize deepseek-tui icon 1024ΓåÆ128 with padding for visual consistency
...
## Improvements
- Security hardening ΓÇö upgrade recommended

# v0.4.36 (2026-05-13)

## Features
- Add MiniMax TTS provider support (#1043)
- Docker images now published on both Docker Hub (`decolua/9router`) and GHCR ΓÇö pull from your preferred registry
