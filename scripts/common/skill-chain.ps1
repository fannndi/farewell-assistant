...
# Usage: . "$PSScriptRoot\skill-chain.ps1"
# Each skill in the chain: @{ name = "skill-name"; desc = "what it does" }
...
$script:SkillChains = @{
    ...
    # ============================================================
    #  ASK CHAINS (simple questions, no execution)
    # ============================================================
    "ask" = @(
...
function Test-SkillChain {
    param($Chain)

    $valid = @()
    $missing = @()
    $skillsDir = "$($script:ROOT_DIR)\ecc\skills"

    foreach ($step in $Chain) {
        $skillPath = Join-Path $skillsDir $step.name
...
function Show-SkillChain {
    param($Chain, [string]$Label = "Skill Chain")

    Write-Host ""
    Write-Host "  $Label ($($Chain.Count) steps):" -ForegroundColor Cyan
    for ($i = 0; $i -lt $Chain.Count; $i++) {
        $step = $Chain[$i]
        $icon = "$($i+1)"
        Write-Host "    $icon. $($step.name)" -ForegroundColor White
